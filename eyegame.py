...
# ====================== 图片/视频/音频路径 ======================
IMG = {
    "clue1_sd": "images/03_线索1_破损存储卡特.jpg",
...
AUDIO = {
    "cover": "audios/bgm_cover.m4a",
    "scene": "audios/bgm_scene.m4a",
    "interrogate": "audios/bgm_interrogate.m4a",
    "judge": "audios/bgm_judge.m4a",
    "win": "audios/bgm_win.m4a",
}
...
if __name__ == "__main__":
    if not st.runtime.exists():
        ...
        st.set_page_config(page_title="眼视光医学侦探", layout="wide", page_icon="🔍")
        ...
        if st.session_state.page == "start":
            ...
            # 音频：自动播放+循环+保留音量/倍速
            try:
                st.audio(AUDIO["cover"], format="audio/m4a", loop=True, autoplay=True)
            except:
                pass
        ...
        # 案发现场
            ...
            try:
                st.audio(AUDIO["scene"], format="audio/m4a", loop=True, autoplay=True)
            except:
                pass
        ...
        # 嫌疑人列表
            ...
            st.title("👥 审问室 · 选择嫌疑人")
            try:
                st.audio(AUDIO["interrogate"], format="audio/m4a", loop=True, autoplay=True)
            except:
                pass
            ...
            with c1:
                ...
                if st.button("审问 张研究员", use_container_width=True):
            ...
            with c2:
                ...
                if st.button("审问 刘实习生", use_container_width=True):
            ...
            with c3:
                ...
                if st.button("审问 王技术员", use_container_width=True):
            ...
            col_a, col_b = st.columns(2)
            with col_a:
                if st.button("前往最终指认", use_container_width=True):
                    jump("judge_page")
                    st.rerun()
            with col_b:
        ...
        # 最终指认
            ...
            try:
                st.audio(AUDIO["judge"], format="audio/m4a", loop=True, autoplay=True)
            except:
                pass
            ...
            with col1:
                if st.button("指认 张研究员", use_container_width=True):
                    jump("win_end")
                    st.rerun()
            with col2:
                if st.button("指认 刘实习生", use_container_width=True):
                    jump("lose1_end")
                    st.rerun()
            with col3:
                if st.button("指认 王技术员", use_container_width=True):
                    jump("lose2_end")
                    st.rerun()

            st.divider()
            if st.button("返回审问室", use_container_width=True):
                jump("sus_list")
                st.rerun()
        ...
        # 成功结局
            ...
            st.title("🎉 真相大白 · 案件侦破")
            try:
                st.audio(AUDIO["win"], format="audio/m4a", loop=True, autoplay=True)
            except:
                pass
