...
if "vid_finish" not in st.session_state:
    st.session_state.vid_finish = False
if "play_bgm" not in st.session_state:
    st.session_state.play_bgm = False
...
.block-container{position:relative;z-index:99;padding:2rem;max-width:100%;}
.suspense-card{background:rgba(0,0,0,0.75);padding:25px;border-radius:16px;margin:12px 0;border:1px solid rgba(255,60,60,0.3);}
h1,h2,h3,p,div{color:white !important;}
.red-text{color:#ff4444;font-weight:bold;}
.green-text{color:#79ff79;font-weight:bold;}
.blue-text{color:#66ccff;font-weight:bold;}
.yellow-text{color:#ffdd66;font-weight:bold;}
.stButton>button{background:linear-gradient(135deg,#b71c1c,#d32f2f);color:white;font-weight:bold;border-radius:10px;padding:12px;border:none;}
...
# 全局BGM
if st.session_state.play_bgm:
    bgm = load_bgm(BGM_PATH)
    if bgm:
        st.audio(bgm, format="audio/m4a", loop=True, autoplay=True)
...
def ai_check_confession_leak(name, answer, ev_list):
    ...
    all_evi = str(ev_list)
    if name == "张研究员":
        if "不接触药剂" in answer and ("商业单据" in all_evi):
            leak += "【漏洞1】自称从不碰麻醉药，但合作单据证明他私下对接药企，具备购药渠道；\n"
        ...
        if "八点离开" in answer and "夜班登记" in all_evi:
            leak += "【漏洞2】答对科普解锁隐藏线索：夜班登记显示他深夜十点仍在院区逗留，离开时间说谎；"
    elif name == "王技术员":
...
""", unsafe_allow_html=True)
        if st.button("开始游戏", use_container_width=True):
            st.session_state.play_bgm = True
            st.session_state.evidence_list.clear()
            st.session_state.leak_analysis = ""
            st.session_state.show_ai_check = False
            st.session_state.q1_ok=st.session_state.q2_ok=st.session_state.q3_ok=st.session_state.q4_ok=st.session_state.q5_ok=False
            jump("scene")
...
# ==========【结局3页面：单独CSS，卡片内文字黑色，视频+文字同步出现】==========
#6成功结局
elif st.session_state.page == "win_end":
    v = load_vid(VIDEO["win"])
    if v:
        st.video(v, autoplay=True, muted=True, loop=False)
    #局部样式：仅当前卡片文字变黑，不影响全局其他页面
    st.markdown("""<style>.end_text h1,.end_text p{color:#000000 !important;}</style>""",unsafe_allow_html=True)
    st.markdown('<div class="suspense-card end_text">', unsafe_allow_html=True)
    st.title("🎉真相大白 · 案件侦破")
    st.markdown("""
你成功抓获张研究员！
他因项目科研经费即将到期，私自修改样本备案，盗取样本卖给校外商业机构获利，利用麻醉剂放倒管理员、关闭监控完成作案，并刻意栽赃另外两人。
""")
    st.markdown('</div>', unsafe_allow_html=True)
